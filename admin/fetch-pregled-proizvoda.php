...
$query = "SELECT * FROM proizvod WHERE obrisan=0";

if (!empty($_POST['query'])) {
    $query .= ' AND naziv LIKE "%' . str_replace(' ', '%', $_POST['query']) . '%"';
}
...
$output = '<div style="padding-left:20px;"><label>Ukupno proizvoda: ' . $total_data . '</label></div>';

$output .= '                  
            <thead>
                <tr>
                    <th class="text-uppercase text-center text-xxs font-weight-bolder" style="color: black;">ID</th>
                    <th class="text-uppercase text-center text-xxs font-weight-bolder" style="color: black;">Naziv</th>
                    <th class="text-uppercase text-center text-xxs font-weight-bolder" style="color: black;">Kategorija</th>
                    <th class="text-uppercase text-center text-xxs font-weight-bolder" style="color: black;">Tip</th>
                    <th class="text-uppercase text-center text-xxs font-weight-bolder" style="color: black;">Poslednja izmena</th>
                    <th class="text-uppercase text-center text-xxs font-weight-bolder" style="color: black;">Detalji / izmena</th>                      
                </tr>
            </thead>';
...
if ($total_filter_data > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        <tr>
            ...
            <td class="align-middle text-center text-sm">
                <h6 class="text-xs mb-0 text-secondary">' . $row['naziv'] . '</h6>
            </td>
            <td class="align-middle text-center text-sm">
                <h6 class="text-xs mb-0 text-secondary">' . $row['kategorija'] . '</h6>
            </td>
            <td class="align-middle text-center text-sm">
                <h6 class="text-xs mb-0 text-secondary">' . $row['tip'] . '</h6>
            </td>
            <td class="align-middle text-center text-sm">
                <h6 class="text-xs mb-0 text-secondary">' . $row['timestamp'] . '</h6>
            </td>
            <td class="align-middle text-center text-sm">
                <a href="detalji-proizvoda.php?naziv_url=' . $row['naziv_url'] . '" class="font-weight-bold">
    ...
    $output .= '
    <tr>
        <td colspan="8" align="center">Nema traženog proizvoda</td>
    </tr>';
}
